...
module.exports = {
  ...
  features: [
    ...
    '**Verified native updates** — cached and ETag-revalidated stable GitHub Release checks verify exact package size and SHA-256, wait up to two minutes for dashboard-tracked active provider sessions and local shells to drain, then hand the exact private-service PID, loopback endpoint, and instance identity to an external rollback-capable helper that revalidates all provider activity and active PTYs before graceful service shutdown, holds an install-scoped lock through replacement and restart verification, leaves every unrelated process untouched, and validates and restarts the new version',
